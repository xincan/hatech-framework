    ...
    >
      ...
      <div slot="hatech-dialog-from">
        ...
        <el-form
          ...
          :rules="form.rules"
        >
          <el-form-item label="角色名称" prop="roleName" :label-width="form.formLabelWidth" >
            <el-input v-model="form.data.roleName" autocomplete="off" placeholder="请输入角色名称" :disabled="form.disabled" :style="{width: form.formInputWidth}"></el-input>
          </el-form-item>

          <el-form-item label="角色描述" prop="description" :label-width="form.formLabelWidth">
            <el-input v-model="form.data.description" autocomplete="off" placeholder="请输入角色描述" :disabled="form.disabled" :style="{width: form.formInputWidth}"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </HatechDialog>
  ...
  export default {
    ...
    ,data() {
      ...
      return {
        ...
        table:{
          title:'角色信息'                         // 表格名称
          ,id:'admin-role-table'        // 表格ID，系统中表格唯一
          ...
          ,column: [                               // 表格头部信息、列的显隐设置
            {label:'角色名称',  prop: 'roleName',   width:'auto', isHide: true}
            ,{label:'描述',  prop: 'description',   width:'auto', isHide: true}
        ...
        this.$refs[this.form.name].validate(valid => {
          ...
          that.$confirm('确定要'+(this.form.data.id ? '修改' : '增加')+'吗?', '温馨提示', {
            ...
            if(result.id === undefined){
              ...
              // 调用父类函数传参
              this.$put("/api/role", result.row).then(response => {
                console.log(result);
              ...
              // 调用父类函数传参
              this.$patch("/api/role", result.row).then(response => {
                console.log(result);
                that.$message({message: response.msg ,center: true ,type: 'success'});
                that.$nextTick(()=>{ that.$refs.hatechTable._initTable(); });
              }).catch(function (error) {
