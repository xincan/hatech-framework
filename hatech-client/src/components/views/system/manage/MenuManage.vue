...
<script>
  ...
  export default {
    ...
    ,data() {
      ...
      return {
        ...
        table:{
          ...
          ,url:'/api/xincan-transaction-system/menu/page'                        // 数据访问路径
    ...
    ,methods: {
      ...
      initSelectTreeMenu(){
        let then = this;
        this.$get("/api/xincan-transaction-system/menu/tree", {}).then(response => {
      ...
      ,delete(result){
        ...
        that.$confirm('确定要删除吗?', '温馨提示', {
          ...
          this.$post("/api/xincan-transaction-system/menu/delete", {
            id:id.substr(1)
        ...
        this.$refs[this.form.name].validate(valid => {
          ...
          that.$confirm('确定要'+(this.form.data.id ? '修改' : '增加')+'吗?', '温馨提示', {
            ...
            if(result.id === undefined){
              ...
              // 调用父类函数传参
              this.$put("/api/xincan-transaction-system/menu", result.row).then(response => {
                console.log(result);
              ...
              // 调用父类函数传参
              this.$patch("/api/xincan-transaction-system/menu", result.row).then(response => {
                console.log(result);
                that.$message({message: response.msg ,center: true ,type: 'success'});
                that.$nextTick(()=>{ that.$refs.hatechTable._initTable(); });
              }).catch(function (error) {
